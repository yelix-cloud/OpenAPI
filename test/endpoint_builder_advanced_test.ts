...
import { assertEquals, assertExists, assertThrows, assertNotEquals } from "jsr:@std/assert";
import type { OpenAPIParameter, OpenAPIResponse, OpenAPIExample, OpenAPILink } from "../src/index.ts";
...
Deno.test("EndpointBuilder advanced features", async (t) => {
  ...
  await t.step("Examples", async (t) => {
    ...
    await t.step("should add parameter named example", () => {
      ...
      assertEquals((param.examples?.activeStatus as OpenAPIExample).value, "active", "Example value should be 'active'");
      assertEquals(param.examples?.activeStatus.summary, "Active status", "Example summary should be set");
    });
    ...
    await t.step("should add response named example", () => {
      ...
      assertExists(mediaType, "Media type should exist");
      assertExists(mediaType.examples, "Examples should exist");
      assertExists(mediaType.examples?.userNotFound, "Named example should exist");
      assertEquals((mediaType.examples?.userNotFound as OpenAPIExample).value, example, "Example value should match");
    });
  });
